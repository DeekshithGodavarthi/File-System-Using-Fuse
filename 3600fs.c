...
static int vfs_rmdir(const char *path)
{
  ...
  /* 3600: NOTE THAT THE BLOCKS CORRESPONDING TO THE FILE SHOULD BE MARKED
	...
	dnode dir2contents;
	cdread(de.block.block, (char*)&dir2contents);
	
	//If the path requested is a directory, remove it
	if(de.type == DIRENTRY_DIR && dir2contents.size == 2){
		inode node;
		cdread(de.block.block, (char *)&node);
		//Remove all blocks that the file contains
		for(unsigned offset = 0; offset < MAX_BLOCKS; ++offset) {
		...
		char * dirn = dir_name(tmp);
		char * filename = file_name(tmp);
		free(tmp);		


		direntry contain_de;
		if(!path_exists(dirn, &contain_de)) {
			//How did you get here?
			return 0;
		}
		free(dirn);

		//Find the directory containing our directory
		dnode containing_dir;
		cdread(contain_de.block.block, (char *)&containing_dir);
		for(unsigned offset = 0; offset < MAX_BLOCKS; ++offset) {
			blocknum block = get_node_block(&containing_dir, offset);
	...
	//Delete cannot remove as files are present in the directory
	}
	else{
		return -ENOTEMPTY;
	}
